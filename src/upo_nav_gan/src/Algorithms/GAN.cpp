...
upo_gan::GAN::GAN(upo_gan::Planning* p, bool norm, float alpha, float inc, int rep) : Learner()
{
	planning_ = p;
...
vector<float> upo_gan::GAN::run_trajectory_diff(int i)
{
...
vector<float> upo_gan::GAN::updateWeights(vector<float> current_weights, vector<float> fc_diff)
{
...
void upo_gan::GAN::updateParamsIter() {
