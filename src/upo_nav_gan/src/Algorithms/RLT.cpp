...
upo_gan::RLT::RLT(upo_gan::Planning* p, bool norm, float alpha, float inc, int rep) : Learner()
{
	planning_ = p;
...
vector<float> upo_gan::RLT::run_trajectory_diff(int i)
{
...
vector<float> upo_gan::RLT::updateWeights(vector<float> current_weights, vector<float> fc_diff)
{
...
void upo_gan::RLT::updateParamsIter() {
	stabilizer_ += stab_incr_;
}
