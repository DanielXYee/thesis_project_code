...
upo_gan::Learning::Learning() {

}

upo_gan::Learning::Learning(upo_gan::Learner* l, int ep, int bs, bool nw, bool ri, string save_dir) {
	learner_ = l;
	epochs_ = ep;
	batch_size_ = bs;
...
void upo_gan::Learning::runLearning()
{
	printf("\n----RUNNING LEARNING-----\n\n");
	...
	printf("\tCalculate feature counts started ====== ");
	//learner_->calculateFeatureCountDemos(); //edited
    printf("\tCalculate feature counts finished ====== ");
	...
	for(unsigned int i=0; i<epochs_; i++)
	{
		...
		for(unsigned int j=0; j<nbatchs; j++)
		{
			...
			printf("Start Executing trajectory");
            ...
            printf("Finished Executing trajectory in this epoch");
			...
			printf("Start recording the weights");
			for(unsigned int w=0; w<weights_.size(); w++) {
				//printf("w%u: %.5f\n", (w+1), weights_[w]);
				outputw << weights_[w] << "\t";
			}
			printf("Finished recording the weights");
			outputw << std::endl;
			printf("\n");
			learner_->setWeights(weights_);
